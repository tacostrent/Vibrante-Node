"""
Lookdev Serializer (Tier 14)
==============================
Persists lookdev data as deterministic, sorted-key JSON with schema versioning.
...
_INSTANCE: Optional[LookdevSerializer] = None
_INSTANCE_LOCK = threading.Lock()


def get_lookdev_serializer() -> LookdevSerializer:
    global _INSTANCE
    if _INSTANCE is None:
        with _INSTANCE_LOCK:
            if _INSTANCE is None:
                _INSTANCE = LookdevSerializer()
    return _INSTANCE


def reset_lookdev_serializer_for_tests() -> None:
    global _INSTANCE
    with _INSTANCE_LOCK:
        _INSTANCE = None
